...
class DiscreteBarChart extends React.Component {
    ...
    createChart(bins) {
        ...
        //   .attr('textLength', d => (d[0].type.length / bins.maxTypeLength) * (margin.bottom - tagPadding))
            ...
            .call(d3.drag()
                ...
                .on('drag', this.TagDrag.bind(this))
    ...
    SelectionDrag(d) {
        ...
        // cases when selection should not move
        if (!this.canDrag) {
            return;
        } else if (dx === 0 && dy === 0) {
            // remain when no move
        ...
        switch(this.direction) {
            case 'x':
                ...
                let value = this.bins.selectedSize();
                let types = this.bins.selectedTypes();

                // find the current position status
                const x = dx + arrange[mappingTable[d.type].x][0].x;
                let index = -1;
                for(let i = 0; i < arrange.length; i += 1) {
                    if(x > arrange[i][0].x - (this.bins.unitWidth / 2) && x < arrange[i][0].x + (this.bins.unitWidth / 2) && i !== mappingTable[d.type].x) {
                        index = i;
                        break;
                    }
                }

                // change on values and types of new Bins
                if (index !== -1) {
                    for (let i = 0; i < arrange[index].length; i += 1) {
                        if (!this.bins.isSelected(arrange[index][i])) {
                            value += arrange[index][i].value;
                            types.push(arrange[index][i].type);
                        }
                    }
                }

                // render tips
                this.updateTip(true, value, types);
                break;
            case 'y':
    ...
    SelectionDrop(e) {
        ...
        } else if (this.direction === 'y' && d3.event.y - this.currentY >= 0) {
            // dragging on y but no displacement
        } else {
